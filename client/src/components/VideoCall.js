...
const VideoCall = ({ user, activeChat, onClose }) => {
  ...
  // Defensive useEffect for attaching streams with debug logs
  useEffect(() => {
    try {
      if (localVideoRef.current && stream && callType === 'video') {
        localVideoRef.current.srcObject = stream;
        console.log('[VideoCall] Attached local video stream');
      }
      if (remoteVideoRef.current && remoteStream && callType === 'video') {
        remoteVideoRef.current.srcObject = remoteStream;
        console.log('[VideoCall] Attached remote video stream');
      }
      if (localAudioRef.current && stream && callType === 'voice') {
        localAudioRef.current.srcObject = stream;
        console.log('[VideoCall] Attached local audio stream');
      }
      if (remoteAudioRef.current && remoteStream && callType === 'voice') {
        remoteAudioRef.current.srcObject = remoteStream;
        console.log('[VideoCall] Attached remote audio stream');
      }
    } catch (err) {
  ...
  function ErrorBoundary({ children }) {
    ...
    if (error) {
      return <div className="p-8 text-red-600 bg-red-100 rounded-lg">An error occurred in the call UI: {error.message}</div>;
    }
    return (
      <React.Suspense fallback={<div className="p-8">Loading call UI...</div>}>
